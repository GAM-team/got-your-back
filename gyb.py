...
import httplib2
from googleapiclient.discovery import V2_DISCOVERY_URI, build, build_from_document
from googleapiclient.http import BatchHttpRequest, MediaIoBaseUpload
...
def getGYBVersion(divider="\n"):
    ...
    return (
        "Got Your Back %s~DIV~%s~DIV~%s - %s~DIV~Python %s.%s.%s %s-bit \
%s~DIV~google-api-client %s~DIV~%s %s"
        % (
            __version__,
            __website__,
            __author__,
...
(troubleshooting)",
    ...
    parser.add_argument("--help", action="help", help="Display this message.")
...
def writeCredentials(creds):
    ...
    if _getValueFromOAuth("iss", creds) not in expected_iss:
        systemErrorExit(
            13,
            "Wrong OAuth 2.0 credentials issuer. Got %s, expected one of %s"
...
#
def readFile(
    filename, mode="r", continueOnError=False, displayError=True, encoding=None
):
    try:
        if filename != "-":
            if not encoding:
...
def doGYBCheckForUpdates(forceCheck=False, debug=False):
    ...
    if not forceCheck:
        ...
        last_check_time = (
            int(last_check_time_str)
            if last_check_time_str and last_check_time_str.isdigit()
            else 0
        )
...
def get_cert_files():
    return (
        os.environ.get("GOOGLE_API_CLIENT_CERTIFICATE"),
    ...
    try:
        ...
        if os.path.isfile(disc_file):
            with open(disc_file, "r") as f:
                discovery = f.read()
            service = build_from_document(discovery, http=httpc)
        else:
    ...
    if os.environ.get("GOOGLE_API_CLIENT_CERTIFICATE") and os.environ.get(
        ...
        if not mtls_root_url:
            if api == "oauth2":
                ...
                mtls_root_url = re.sub(
                    r"googleapis\.com", "mtls.googleapis.com", root_url
                )
        ...
        service._add_basic_methods(
            service._resourceDesc, service._rootDesc, service._schema
        )
    return service
...
def callGAPI(
    service, function, soft_errors=False, throw_reasons=[], retry_reasons=[], **kwargs
):
    retries = 10
            ...
            if message_attribute:
                try:
                    show_message = show_message.replace(
                        "%%first_item%%", str(this_page[items][0][message_attribute])
                    )
                    show_message = show_message.replace(
                        "%%last_item%%", str(this_page[items][-1][message_attribute])
                    )
                except (IndexError, KeyError):
...
def getValidateLoginHint(login_hint):
    ...
    while True:
        login_hint = input(
            "\nWhat is your Google Workspace admin email address? "
        ).strip()
        if VALIDEMAIL_PATTERN.match(login_hint):
            return login_hint
...
def shorten_url(long_url):
    ...
    headers = {"Content-Type": "application/json", "User-Agent": getGYBVersion(" | ")}
    try:
        resp, content = simplehttp.request(
            url_shortnr, "POST", f'{{"long_url": "{long_url}"}}', headers=headers
        )
...
def _localhost_to_ip():
    ...
    if (
        local_ipaddress not in ip4_local_range
        and local_ipaddress not in ip6_local_range
    ):
        local_ip = "127.0.0.1"
    return local_ip
...
MESSAGE_LOCAL_SERVER_SUCCESS = (
    "The authentication flow has completed. You may"
...
GYB_PROJECT_APIS = (
    "https://raw.githubusercontent.com/jay0lee/got-your-back/master/project-apis.txt?"
)
    ...
    for api in apis:
        while True:
            ...
            try:
                ...
                print(
                    "\nThere was an error enabling %s. Please resolve error as described below:"
                    % api
                )
                print
                print("\n%s\n" % e)
...
def _getLoginHintProjects():
    ...
    if not pfilter:
        ...
        print(
            "ERROR: delete-projects action requires --email and a project --search argument"
        )
        sys.exit(3)
...
def doCreateProject():
    ...
    for a_file in [service_account_file, client_secrets_file]:
        if os.path.exists(a_file):
            print(
                "File %s already exists. Please delete or rename it before attempting to create another project."
                % a_file
            )
            sys.exit(5)
    login_hint = options.email
    ...
    for i in range(3):
        project_id += "-%s" % "".join(
            random.choice(string.digits + string.ascii_lowercase) for i in range(3)
        )
    ...
    while True:
        ...
        for i in range(1, 5):
            ...
            status = callGAPI(crm.operations(), "get", name=operation_name)
            if "error" in status:
                if (
                    ...
                    org_policy["bindings"].append(
                        {"role": my_role, "members": ["user:%s" % login_hint]}
                    )
                    callGAPI(
                        crm.organizations(),
                        "setIamPolicy",
                ...
                try:
                    if (
                        status["error"]["details"][0]["violations"][0]["description"]
...
and accept the Terms of Service (ToS). As soon as you've accepted the ToS popup, you can return here and press enter.""")
    ...
    sa_body = {
        ...
        "serviceAccount": {"displayName": "GYB Project Service Account"},
    }
    service_account = callGAPI(
...
def getSvcAcctCredentials(scopes, act_as):
    try:
        json_string = readFile(
            os.path.join(options.config_folder, "oauth2service.json"),
            continueOnError=True,
            displayError=True,
        )
        if not json_string:
        ...
        credentials = (
            google.oauth2.service_account.Credentials.from_service_account_info(sa_info)
        )
...
def doCheckServiceAccount():
    ...
    for scope in all_scopes:
        try:
            credentials = getSvcAcctCredentials(
                [scope, "https://www.googleapis.com/auth/userinfo.email"], options.email
            )
            granted_scopes = callGAPI(oa2, "tokeninfo", access_token=credentials.token)
            if (
                scope in granted_scopes["scope"].split(" ")
                and granted_scopes.get("email", "").lower() == options.email.lower()
            ):
                result = "PASS"
            else:
                result = "FAIL"
...
def check_db_settings(db_settings, action, user_email_address):
    if (
        db_settings["db_version"] < __db_schema_min_version__
        or db_settings["db_version"] > __db_schema_version__
    ):
        print(
            "\n\nSorry, this backup folder was created with version %s of the \
database schema while GYB %s requires version %s - %s for restores"
            % (
                db_settings["db_version"],
                __version__,
                __db_schema_min_version__,
                __db_schema_version__,
            )
        )
        sys.exit(4)
...
account that it was created with for incremental backups. You specified the\
 %s account"
                % (db_settings["email_address"], user_email_address)
            )
            sys.exit(5)
    ...
    try:
        ...
        """)
            ...
            sqlconn.executemany(
                ...
                (("uidvalidity", uidvalidity), ("db_version", __db_schema_version__)),
            )
            sqlconn.commit()
    except sqlite3.OperationalError as e:
...
def labelIdsToLabels(labelIds):
    ...
    for labelId in labelIds:
        if labelId not in allLabelIds:
            ...
            label_results = callGAPI(
                gmail.users().labels(),
                "list",
                userId="me",
                fields="labels(name,id,type)",
            )
            allLabelIds = dict()
...
def labelsToLabelIds(labels):
    ...
    if len(allLabels) < 1:  # first fetch of all labels from Google
        label_results = callGAPI(
            gmail.users().labels(), "list", userId="me", fields="labels(name,id,type)"
        )
        allLabels = dict()
        for a_label in label_results["labels"]:
            if a_label["type"] == "system":
...
def refresh_message(request_id, response, exception):
    if exception is not None:
        ...
        if "labelIds" in response:
            ...
            sqlcur.execute(
                """DELETE FROM labels where message_num =
                   (SELECT message_num from uids where uid = ?)
                    AND label NOT IN current_labels""",
                ((response["id"]),),
            )
            sqlcur.execute(
                """INSERT INTO labels (message_num, label)
...
def backup_chat(request_id, response, exception):
    ...
    html = base64.urlsafe_b64decode(
        response["payload"]["body"].get("data", "")
    ).decode()
    ...
    try:
        ...
        time_for_sqlite = datetime.datetime.fromtimestamp(
            86400
        )  # minimal value Win accepts
    message_rel_path = os.path.join(
        str(message_date.tm_year), str(message_date.tm_mon), str(message_date.tm_mday)
    )
    message_rel_filename = os.path.join(message_rel_path, message_file_name)
    message_full_path = os.path.join(options.local_folder, message_rel_path)
    ...
    sqlcur.execute(
        ...
        (message_rel_filename, time_for_sqlite),
    )
...
def getSizeOfMessages(messages, gmail):
    ...
    for a_message in messages:
        gbatch.add(
            gmail.users()
            .messages()
            .get(userId="me", id=a_message, format="minimal", fields="id,sizeEstimate"),
        ...
        if len(gbatch._order) == options.batch_size:
            ...
            gbatch = gmail.new_batch_http_request()
            rewrite_line(
                "Estimated size %s %s/%s messages"
                % (bytes_to_larger(running_size), estimated_messages, estimate_count)
            )
...
def restore_msg_to_group(gmig, full_message, message_num, sqlconn):
    ...
    media = MediaIoBaseUpload(
        fstr, mimetype="message/rfc822", chunksize=-1, resumable=True
    )
    try:
        callGAPI(
            gmig.archive(),
            "insert",
            groupId=options.email,
            media_body=media,
            soft_errors=True,
        )
    except googleapiclient.errors.MediaUploadSizeError as e:
...
def main(argv):
    ...
    if not options.email:
        ...
        sys.exit(1)
    if (
        options.action in ["restore", "restore-group", "restore-mbox"]
        and options.gmail_search != "-is:chat"
    ):
    ...
    if not os.path.isdir(options.local_folder):
        if options.action in ["backup", "backup-chat"]:
            ...
            print(
                "ERROR: Folder %s does not exist. Cannot restore."
    ...
    if options.action == "backup":
        ...
        messages_to_process = callGAPIpages(
            ...
            maxResults=500,
            userId="me",
            includeSpamTrash=options.spamtrash,
            q=options.gmail_search,
        ...
        for message_num in messages_to_process:
            if not newDB and message_is_backed_up(
                message_num["id"], sqlcur, sqlconn, options.local_folder
            ):
                messages_to_refresh.append(message_num["id"])
            else:
                messages_to_backup.append(message_num["id"])
        print(
            "GYB already has a backup of %s messages"
        ...
        for a_message in messages_to_backup:
            ...
            if len(gbatch._order) == options.batch_size or (
                options.memory_limit and request_size >= memory_limit
            ):
                ...
                if options.memory_limit:
                    request_size = message_sizes[a_message]
                rewrite_line(
                    "backed up %s of %s messages" % (backed_up_messages, backup_count)
                )
            gbatch.add(
                gmail.users()
                .messages()
    ...
    """)
        ...
        for a_message in messages_to_refresh:
            gbatch.add(
                gmail.users()
                .messages()
                .get(userId="me", id=a_message, format="minimal", fields="id,labelIds"),
            ...
            if len(gbatch._order) == options.batch_size:
                ...
                sqlconn.commit()
                rewrite_line(
                    "refreshed %s of %s messages" % (refreshed_messages, refresh_count)
                )
        if len(gbatch._order) > 0:
            callGAPI(gbatch, None, soft_errors=True)
            sqlconn.commit()
    ...
    # BACKUP-CHAT
        ...
        for message_num in messages_to_process:
            if newDB or not message_is_backed_up(
                message_num["id"], sqlcur, sqlconn, options.local_folder
            ):
                messages_to_backup.append(message_num["id"])
        print(
            "GYB already has a backup of %s Chats"
            % (len(messages_to_process) - len(messages_to_backup))
        )
        backup_count = len(messages_to_backup)
        ...
        for a_message in messages_to_backup:
            ...
            if len(gbatch._order) == options.batch_size or (
                ...
                if options.memory_limit:
                    request_size = message_sizes[a_message]
                rewrite_line(
                    "backed up %s of %s Chats" % (backed_up_messages, backup_count)
                )
            gbatch.add(
                gmail.users()
                .messages()
    ...
    # RESTORE #
        ...
        resumedb = os.path.join(
            options.local_folder, "%s-restored.sqlite" % options.email
        )
        if options.noresume:
            try:
                os.remove(resumedb)
      ...
      message_filename FROM messages
        ...
        for x in messages_to_restore_results:
            ...
            if not os.path.isfile(os.path.join(options.local_folder, message_filename)):
                print(
                    "WARNING! file %s does not exist for message %s"
                    % (
            ...
            if not options.strip_labels:
                sqlcur.execute(
                    "SELECT DISTINCT label FROM labels WHERE message_num \
          = ?",
                    (message_num,),
                )
            ...
            if b64_message_size > 1 * 1024 * 1024 or options.batch_size == 1:
                ...
                try:
                    ...
                    exception = e
                restored_message(
                    request_id=str(message_num), response=response, exception=exception
                )
                rewrite_line(
                    "restored single large message (%s/%s)" % (current, restore_count)
                )
                continue
        ...
        if not options.strip_labels:
            for path, subdirs, files in os.walk(options.local_folder):
                for filename in files:
                    ...
                    print(
                        "\nReading Vault labels from %s file %s"
                        % (humansize(file_path), file_path)
                    )
            ...
            for filename in files:
                ...
                while True:
                    ...
                    request_id = hashlib.md5(
                        message_marker.encode("utf-8")
                    ).hexdigest()[:25]
                    if request_id in messages_to_skip:
                        rewrite_line(
                            " skipping already restored message #%s" % (current,)
                        )
                        try:
                            mbox.skip()
                    ...
                    if not options.strip_labels:
                        if vault_label_map:
                            ...
                            msg_id = message.get_header(
                                b"message-id", case_insensitive=True
                            )[1:-2]
                        ...
                        labels.extend(
                            [
                                p.strip(string.whitespace + '"')
                                for p in re.split("(,|\\\".*?\\\"|'.*?')", labels_str)
                                if p.strip(",")
                            ]
                        )
                    cased_labels = []
                    ...
                    if b64_message_size > 1 * 1024 * 1024:
                        ...
                        rewrite_line(
                            " restoring %s message %s - %s%%"
                            % (humansize(b64_message_size), current, mbox_pct)
                        )
                        fstr = BytesIO(full_message)
                        media = googleapiclient.http.MediaIoBaseUpload(
                            fstr,
                    ...
                    if len(gbatch._order) > 0 and current_batch_bytes > max_batch_bytes:
                        ...
                        rewrite_line(
                            " restoring %s messages %s - %s%%"
                            % (len(gbatch._order), current, mbox_pct)
                        )
                        callGAPI(gbatch, None, soft_errors=True)
                        gbatch = gmail.new_batch_http_request()
                        sqlconn.commit()
    ...
    # RESTORE-GROUP #
        ...
        max_message_size = gmig._rootDesc["resources"]["archive"]["methods"]["insert"][
            "mediaUpload"
        ]["maxSize"]
        ...
        message_filename FROM messages
            ...
            for x in messages_to_restore_results:
                current += 1
                rewrite_line(
                    "restoring message %s of %s from %s"
                ...
                if not os.path.isfile(
                    ...
                    continue
                with open(
                    os.path.join(options.local_folder, message_filename), "rb"
                ) as f:
                    full_message = f.read()
                if options.cleanup:
                    full_message = message_hygiene(full_message)
    ...
    # PURGE-LABELS #
        ...
        existing_labels = callGAPI(
            gmail.users().labels(), "list", userId="me", fields="labels(id,name,type)"
        )
        for label_result in existing_labels["labels"]:
            if label_result["type"] == "system" or not pattern.search(
                label_result["name"]
            ):
                continue
            try:
                ...
                printable_name = "".join(
                    c for c in label_result["name"] if c in safe_chars
                )
    ...
    # PRINT-LABELS #
        ...
        all_system_labels = [
            label for label in labels.get("labels") if label["type"] == "system"
        ]
        all_user_labels = [
            label for label in labels.get("labels") if label["type"] != "system"
        ]
        ...
        for label_list in [all_system_labels, all_user_labels]:
            for label in label_list:
                try:
                    ...
                    printable_name = "".join(
                        c for c in label["name"] if c in safe_chars
                    )
...
otaBytesByService,quotaType",
    ...
    # REVOKE
        ...
        revoke_uri = "https://accounts.google.com/o/oauth2/revoke?%s" % urlencode(
            params
        )
    ...
    # ESTIMATE #
        ...
        for message_num in messages_to_process:
            if (
                not newDB
                and os.path.isfile(sqldbfile)
                and message_is_backed_up(
                    message_num["id"], sqlcur, sqlconn, options.local_folder
                )
            ):
                pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            """ERROR: GYB ran out of memory during %s. Try the following:

1) Use a 64-bit version of GYB. It has access to more memory.
2) Add "--memory-limit 100" argument to GYB to reduce memory usage."""
            % options.action
        )
        sys.exit(5)
        ...
        if e.reason == "NO_PROTOCOLS_AVAILABLE":
            print(
                "ERROR: %s - Please adjust your --tls-min-version and --tls-max-version arguments."
                % e.reason
            )
        else:
            raise
