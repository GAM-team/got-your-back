...
def getValidOauth2TxtCredentials(force_refresh=False):
    ...
    if (credentials and credentials.expired) or force_refresh:
        ...
        for n in range(1, retries+1):
            try:
                credentials.refresh(
                    google_auth_httplib2.Request(_createHttpObj()))
...
def doGYBCheckForUpdates(forceCheck=False, debug=False):
    ...
    last_update_check_file = os.path.join(
        options.config_folder, 'lastcheck.txt')
...
def buildGAPIObject(api, httpc=None):
    if not httpc:
        ...
        httpc = google_auth_httplib2.AuthorizedHttp(
            credentials, _createHttpObj())
    if options.debug:
    ...
    try:
        ...
        disc_file = os.path.join(
            options.config_folder, '%s-%s.json' % (api, version))
    ...
    if checkEnabled:
        ...
        for enabled in enabledServices:
            ...
            if service_name in apis:
                ...
                print(' API %s (non-GYB) is enabled (which is fine)' %
                      service_name)
    for api in apis:
        while True:
...
# If the newlines are not present, the data is correctly read with the first input
    ...
    client_secrets_file = os.path.join(
        options.config_folder, 'client_secrets.json')
...
def _getLoginHintProjects():
    ...
    client_secrets_file = os.path.join(
        options.config_folder, 'client_secrets.json')
    if pfilter == 'current':
        cs_data = readFile(client_secrets_file, mode='rb',
                           continueOnError=True, displayError=True, encoding=None)
        if not cs_data:
            systemErrorExit(
                14, 'Your client secrets file:\n\n%s\n\nis missing. Please recreate the file.' % client_secrets_file)
        try:
            cs_json = json.loads(cs_data)
...
def doDelProjects():
    ...
    for project in projects:
        ...
        callGAPI(crm.projects(), 'delete',
                 projectId=projectId, soft_errors=True)
...
def doCreateProject():
    service_account_file = os.path.join(
        options.config_folder, 'oauth2service.json')
    client_secrets_file = os.path.join(
        options.config_folder, 'client_secrets.json')
    ...
    for i in range(3):
        project_id += '-%s' % ''.join(random.choice(string.digits +
...
def getSvcAcctCredentials(scopes, act_as):
    try:
        json_string = readFile(os.path.join(
            options.config_folder, 'oauth2service.json'), continueOnError=True, displayError=True)
        if not json_string:
...
def doCheckServiceAccount():
    ...
    for scope in all_scopes:
        try:
            ...
            granted_scopes = callGAPI(
                oa2, 'tokeninfo', access_token=credentials.token)
    ...
    for message_num, filename in sqlconn.execute('''
        ...
        if os.path.isfile(message_full_filename):
            with open(message_full_filename, 'r') as f:
                msgid = header_parser.parse(f, True).get(
                    'message-id') or '<DummyMsgID>'
            sqlcur.execute(
...
def createLabel(label_name):
    ...
    if label_name in allLabels:
        sys.stderr.write(
            f'WARNING: refusing to recreate existing label {label_name}\n')
        return
    ...
    try:
        ...
        sys.stderr.write(
            f'WARNING: failed to create (existing?) label {label_name}\n')
...
def backup_chat(request_id, response, exception):
    ...
    html = base64.urlsafe_b64decode(
        response['payload']['body'].get('data', '')).decode()
    ...
    try:
        ...
        time_for_sqlite = datetime.datetime.fromtimestamp(
            86400)  # minimal value Win accepts
...
def message_hygiene(msg):
    ...
    if not orig_date:
        ...
        try:
            new_date_gmt = email.utils.format_datetime(
                parsed_datetime, usegmt=True)
        except ValueError:
    ...
    if gyb_changes:
        ...
        omsg.add_header('X-GYB-Changes-Made',
...
def main(argv):
    ...
    if options.version:
        ...
        print('gmail.googleapis.com connects using %s %s' %
              (tls_ver, cipher_name))
        sys.exit(0)
            ...
            for filename in files:
                ...
                while True:
                    ...
                    request_id = hashlib.md5(
                        message_marker.encode('utf-8')).hexdigest()[:25]
                    ...
                    if not options.strip_labels:
                        ...
                        mybytes, encoding = email.header.decode_header(labels_str)[
                            0]
                        if encoding != None:
                            try:
                                ...
                                pass
                        labels.extend([p.strip(string.whitespace+'\"') for p in re.split(
                            "(,|\\\".*?\\\"|'.*?')", labels_str) if p.strip(',')])
                    cased_labels = []
                    for label in labels:
                    ...
                    if labelIds:
                        body['labelIds'] = labelIds
                    b64_message_size = (len(full_message)/3) * 4
                    rewrite_line(" reading message %s... - %s%%" %
                                 (current, mbox_pct))
                    if b64_message_size > 1 * 1024 * 1024:
                        ...
                        restored_message(request_id=request_id, response=response,
                                         exception=exception)
                        rewrite_line(
                            " restored single large message (%s)" % (current,))
                        continue
                    raw_message = base64.urlsafe_b64encode(
                        full_message).decode('utf-8')
                    body['raw'] = raw_message
                    ...
                    if len(gbatch._order) == options.batch_size:
                        ...
                        largest_in_batch = 0
                if len(gbatch._order) > 0:
                    rewrite_line("restoring %s messages (%s)" %
                                 (len(gbatch._order), current,))
                    callGAPI(gbatch, None, soft_errors=True)
                    sqlconn.commit()
        print('\ndone!')
        sqlconn.commit()
    ...
    # RESTORE-GROUP #
        ...
        gyb_format = os.path.isfile(os.path.join(
            options.local_folder, 'msg-db.sqlite'))
        if gyb_format:
            sqlcur.execute('ATTACH ? as resume', (resumedb,))
            sqlcur.executescript('''CREATE TABLE IF NOT EXISTS resume.restored_messages
                        (message_num INTEGER PRIMARY KEY);
                ...
                for filename in files:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            break
                        mbox_pct = percentage(
                            mbox._mbox_position, mbox._mbox_size)
                        rewrite_line(" message %s - %s%%" %
                                     (current, mbox_pct))
                        full_message = message.as_bytes()
                        if options.cleanup:
                            full_message = message_hygiene(full_message)
                        restore_msg_to_group(
                            gmig, full_message, request_id, sqlconn)
        sqlconn.commit()
        sqlconn.execute('DETACH resume')
        sqlconn.commit()
    ...
    # PRINT-LABELS #
        ...
        labels = callGAPI(gmail.users().labels(), 'list',
                          userId='me', fields='labels(id,name,type)')
        all_system_labels = [label for label in labels.get(
            'labels') if label['type'] == 'system']
        all_user_labels = [label for label in labels.get(
            'labels') if label['type'] != 'system']
...
2) Add "--memory-limit 100" argument to GYB to reduce memory usage.''' % options.action)
        ...
        if e.reason == 'NO_PROTOCOLS_AVAILABLE':
            print(
                'ERROR: %s - Please adjust your --tls-min-version and --tls-max-version arguments.' % e.reason)
        else:
            raise
    except KeyboardInterrupt:
        try:
